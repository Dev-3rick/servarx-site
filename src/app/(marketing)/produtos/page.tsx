...
export default function ProdutosPage() {
  return (
    <>
        ...
        <div className="max-w-page-xl mx-auto">
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {PRODUTOS.map((p, i) => {
              ...
              const cardClasses = `rounded-3xl border bg-surface p-8 flex flex-col transition-all h-full ${
                interactive
              ...
              const cardContent = (
                <>
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <span className={`text-xs font-mono uppercase tracking-wider rounded-full px-3 py-1 ${status.bg} ${status.fg}`}>
                      {status.label}
                    </span>
                  </div>
                  <h3 className="text-2xl font-bold text-brand-teal-800 mb-2">{p.nome}</h3>
                  <div className="mb-4">
                  ...
                  <p className="text-brand-neutral-muted leading-relaxed flex-1">{p.descricao}</p>
                  {interactive && (
              ...
              return (
                <Reveal key={p.nome} variant="up" delay={i * 200}>
                  {interactive ? (
                    <Link href={p.href!} className={cardClasses}>
                      {cardContent}
                    </Link>
                  ) : (
      ...
      {/* "Tem outra vertical?" — DARK */}
      <section className="relative overflow-hidden bg-brand-teal-800 text-white">
        <div className="absolute inset-0 opacity-50" style={{ background: 'var(--gradient-hero-spot)' }} aria-hidden="true" />
        <div className="relative max-w-page-md mx-auto px-6 lg:px-12 py-24 lg:py-32 text-center">
          <Reveal variant="up">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-full bg-brand-cyan-400 text-brand-teal-800 px-8 py-4 font-semibold shadow-cta-accent hover:bg-white hover:scale-105 transition-all"
            >
